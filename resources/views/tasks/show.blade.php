    ...
    <div class="w-full max-w-7xl mx-auto mb-10 mt-6">
        ...
        <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div class="flex gap-3">
                ...
                <form action="{{ route('tasks.destroy', $task) }}" method="POST" onsubmit="return confirm('Bu görevi silmek istediğinize emin misiniz?');">
                    @csrf
                    @method('DELETE')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-card-dark rounded-2xl border border-secondary/30 p-6 md:p-8 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                    <div class="flex flex-wrap gap-3 mb-6">
                        ...
                        @if($task->priority === 'high')
                            <span class="bg-red-500/20 text-red-400 border border-red-500/20 rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider">Acil (Yüksek)</span>
                        ...
                        @else
                            <span class="bg-green-500/20 text-green-400 border border-green-500/20 rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider">Düşük</span>
                        @endif

                        <!-- Status Badge -->
                        @if($task->status === 'completed')
                            <span class="bg-green-500/20 text-green-400 border border-green-500/20 rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider shadow-[0_0_5px_rgba(74,222,128,0.3)]">Tamamlandı</span>
                        @else
                            <span class="bg-[#e4ba00]/20 text-[#e4ba00] border border-[#e4ba00]/30 rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider shadow-[0_0_5px_rgba(228,186,0,0.3)]">Bekliyor</span>
                        @endif
                    </div>
                    ...
                    <div class="prose prose-invert max-w-none text-secondary">
                        <p class="whitespace-pre-wrap">{{ $task->description ?: 'Bu görev için bir açıklama girilmemiş.' }}</p>
                    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-card-dark rounded-2xl border border-secondary/30 p-6 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                    <h3 class="text-lg font-bold text-white mb-4">Personel Bilgileri</h3>
                    
                    <div class="space-y-4">
                        <div>
                            <p class="text-xs text-secondary mb-1">Atanan Personel</p>
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold">
                                    {{ $task->assignedUser ? substr($task->assignedUser->name, 0, 1) : '?' }}
                                </div>
                                <div>
                                    <p class="text-sm font-semibold text-white">{{ $task->assignedUser->name ?? 'Atanmamış' }}</p>
                                    <p class="text-xs text-secondary">{{ $task->assignedUser->email ?? 'Havuz Görevi' }}</p>
                                </div>
                            </div>
                        </div>
                        
                        <div class="border-t border-secondary/20 pt-4">
                            <p class="text-xs text-secondary mb-1">Oluşturan Yönetici</p>
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-semibold text-white">{{ $task->manager->name ?? 'Sistem' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Tarih Bilgileri -->
                <div class="bg-card-dark rounded-2xl border border-secondary/30 p-6 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                    <h3 class="text-lg font-bold text-white mb-4">Zaman Çizelgesi</h3>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <p class="text-xs text-secondary">Oluşturulma Tarihi</p>
                ...
                <div class="bg-card-dark rounded-2xl border border-secondary/30 p-6 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                    ...
                    @if($task->auditPoint)
                        ...
                        <p class="text-sm text-secondary mt-3">
                            <span class="font-medium text-text">Konum:</span> {{ $task->auditPoint->name }}
                        </p>
                    @else
                        <div class="flex flex-col items-center justify-center p-8 bg-background/50 rounded-xl border border-secondary/20">
                            ...
                            <p class="text-secondary text-center">Bu görev için konum bilgisi bulunmuyor.</p>
                        </div>
                    @endif
                </div>
            </div>
    ...
    @push('scripts')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                // Haritadaki kaydırmayı kapatabiliriz (opsiyonel, statik görünüm için)
